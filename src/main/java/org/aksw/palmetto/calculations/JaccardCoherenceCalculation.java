...
public class JaccardCoherenceCalculation extends AbstractSubsetCreatorBasedCoherenceCalculation {

    public JaccardCoherenceCalculation(SubsetCreator subsetCreator) {
        super(subsetCreator);
    }
            ...
            if (segmentProbability > 0) {
                for (int j = 0; j < subsetProbabilities.conditions[i].length; ++j) {
                    ...
                    if (joinProbability > 0) {
                        coherence += intersectionProbability / joinProbability;
                    }
                }
            }
