...
public class RatioCoherenceCalculation extends AbstractSubsetCreatorBasedCoherenceCalculation {

    public RatioCoherenceCalculation(SubsetCreator subsetCreator) {
        super(subsetCreator);
    }
            ...
            if (segmentProbability > 0) {
                for (int j = 0; j < subsetProbabilities.conditions[i].length; ++j) {
                    ...
                    if ((conditionProbability > 0) && (jointProbability > 0)) {
                        coherence += jointProbability / (segmentProbability * conditionProbability);
                    }
                }
            }
